...
int main(int argc, char** argv) {
    ...
    if((argc == 2 && strcmp(argv[1], "--help") == 0)) {
        ...
        if(argc < 8) {// Check that the number of inserted parameters is correct
            if (myrank == 0) print_error_and_exit("Please insert all the necessary parameters (for a complete description use --help option)");
        } else { 
            ...
            if (!train_file || !test_file) 
                print_error_and_exit("Error when trying to open the files. Please retry");
            ...
            if (N_TRAIN <= 0 || N_TEST <= 0 || K <= 0 || N_FEATURES <= 0 || N_CLASSES <= 0)
                print_error_and_exit("All the numeric parameters must assume positive values");

            if (N_FEATURES > NMAX_FEATURES)
                print_error_and_exit("The maximum number of features that a sample can have is 255");
